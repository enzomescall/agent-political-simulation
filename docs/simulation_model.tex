...
$t$ & \texttt{models/world.py} & 0 & Turn counter. \\
$\mathbf{i}_a$ & \texttt{models/agent.py} & --- & Ideological position per axis in $[-1,1]$. \\
$\alpha_{a,g}$ & \texttt{models/agent.py} & \{\} & IG influence weights on agent actions/votes. \\
$\pi_{a,g}$ & \texttt{models/agent.py} & \{\} & IG-specific approval that affects elections/vulnerability. \\
$r_{a,b}$ & \texttt{models/agent.py} & \{\} & Inter-agent trust used in votes and actions. \\
$s_a$ & \texttt{models/agent.py} & 0.5 & Loyalty/standing inside party; used in voting/expulsion risk. \\
$\eta_a$ & \texttt{models/agent.py} & 0.5 & Noise scale for action choice; affects randomness. \\
$\mathbf{w}_a$ & \texttt{actions/utility.py} & \texttt{DEFAULT\_WEIGHTS} & Per-agent action utility coefficients. \\
$\omega_{a,c}$ & \texttt{actions/voting.py} & (generated) & Per-agent vote component coefficients. \\

$\theta_k$ & \texttt{models/party.py} & 0.15 & Party-line activation threshold for directives. \\
$B_k$ & \texttt{models/party.py} & 3 & Election nomination budget decrementing on use. \\
--- & \texttt{models/party.py} & 0.3 & Nomination threshold. \\
--- & \texttt{models/party.py} & 20 & Leadership election interval. \\

$\tau_p$ & \texttt{models/government.py} & -0.3 & Executive veto if exec disposition below threshold. \\
$\phi_p$ & \texttt{models/government.py} & 2/3 & Fraction of seats needed (plus 1) to override veto. \\
--- & \texttt{simulation/elections.py} & --- & Per-government election schedule interval. \\

--- & \texttt{actions/utility.py} & (0.3,0.2,0.15,0.2,0.15) & Default utility weights \texttt{DEFAULT\_WEIGHTS}. \\

--- & \texttt{actions/voting.py} & varies & Archetype vote weights \texttt{ARCHETYPE\_WEIGHTS}. \\
--- & \texttt{actions/voting.py} & $\pm 0.1$ & Vote thresholds for YES/NO/abstain logic. \\

$\sigma$ & \texttt{simulation/drift.py} & 0.01 & Std-dev of per-turn ideology Brownian noise. \\
$\lambda_{IG}$ & \texttt{simulation/drift.py} & 0.02 & Pull rate toward top-allegiance IG ideology. \\
$\delta$ & \texttt{simulation/drift.py} & 0.4 & Divergence threshold before party-pull activates. \\
$\lambda_{party}$ & \texttt{simulation/drift.py} & 0.015 & Pull rate back toward party ideology. \\
$\sigma_{rad}$ & \texttt{simulation/drift.py} & 0.25 & Avg satisfaction below triggers IG radicalization. \\
$\gamma$ & \texttt{simulation/drift.py} & 1.05 & Per-turn multiplier pushing IG ideology away from zero. \\
$\kappa$ & \texttt{simulation/drift.py} & 1.02 & Per-turn multiplier increasing IG electorate share. \\
$\sigma_{cent}$ & \texttt{simulation/drift.py} & 0.72 & Avg satisfaction above triggers IG moderation. \\
--- & \texttt{simulation/drift.py} & 0.03 & Centrist pull rate. \\
--- & \texttt{simulation/drift.py} & 0.99 & Centrist share decay. \\

--- & \texttt{simulation/elections.py} & 2 & Executive term limit. \\
--- & \texttt{simulation/elections.py} & 4 & Legislative term limit. \\
--- & \texttt{simulation/elections.py} & 10 & Stale-agent removal threshold. \\
--- & \texttt{simulation/elections.py} & 0.25 & Support dilution factor. \\
--- & \texttt{simulation/elections.py} & 0.6 & Viability ratio. \\

--- & \texttt{simulation/factions.py} & 0.45 & Split satisfaction threshold. \\
--- & \texttt{simulation/factions.py} & 3 & Split inertia. \\
--- & \texttt{simulation/factions.py} & 0.10 & Split ideology gap. \\
--- & \texttt{simulation/factions.py} & 2 & Minimum splinter size. \\
--- & \texttt{simulation/factions.py} & 2 & Splinter start budget. \\
--- & \texttt{simulation/factions.py} & 0.5 & Ambition floor. \\
